...
PLUGIN_SOLVER_ROOT = Path(__file__).resolve().parents[1]
...
def _problem_dir(problem_id: str) -> Path:
    ...
    run_root = (RUNS_ROOT / sanitized_problem_id).resolve()
    runs_root = RUNS_ROOT.resolve()
    if not run_root.is_relative_to(runs_root):
        raise ValueError("problem_id resolves outside Rethlas runs root")
...
def memory_init(
    ...
    return {
        "problem_id": sanitized_problem_id,
        "run_dir": str(problem_dir.parent),
