...
description: Validate externally referenced theorems by querying arXiv theorem search first and Claude Code WebSearch second. Use when a markdown proof cites statements from external papers.
---
...
12. If no match is found, use Claude Code `WebSearch` with the same statement text.
...
```json
{
  ...
  "critical_error": {
    "location": "Lemma 2",
    "issue": "Referenced external theorem was not found in arXiv search or WebSearch."
  }
}
```
